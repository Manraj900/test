...
let convertRunLengthToHoursAndMinutes (minutes: int) : string =
...
let movieRunLengthsWithNames =
    oscarMovies
    |> List.map (fun movie -> sprintf "%s: %s" movie.Name (convertRunLengthToHoursAndMinutes movie.RunLength))

printfn "Probable Oscar Winners:"
probableOscarWinners
|> List.iter (fun movie -> printfn "%s" movie.Name)

printfn "\nMovie Run Lengths in Hours and Minutes:"
movieRunLengthsWithNames
|> List.iter (printfn "%s")
